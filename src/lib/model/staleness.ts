...
/** 0 fresh, 1 yellowing (>7d), 2 cracked (>14d), 3 cobwebbed (>30d). */
...
export function staleStage(touchedAt: number, now = Date.now()): StaleStage {
  const d = daysSince(touchedAt, now);
...
export const AGE_GROUPS: { key: string; label: string; test: (days: number) => boolean }[] = [
  { key: 'today', label: 'Touched today', test: (d) => d < 1 },
...
export function ageGroupKey(touchedAt: number, now = Date.now()): string {
  const d = daysSince(touchedAt, now);
